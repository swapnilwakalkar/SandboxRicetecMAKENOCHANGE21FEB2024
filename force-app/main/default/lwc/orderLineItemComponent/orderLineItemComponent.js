...
import retrieveAllProducts from '@salesforce/apex/ProductsRetrivingfororderClass.retrieveProducts';
import updateOrderProducts  from '@salesforce/apex/ProductsRetrivingfororderClass.UpdateOrderProducts';
...
const columns = [
    { type: 'checkbox', label: 'Select', initialWidth: 50, typeAttributes: { rowIndex: 'index', selectedRows: [] } },
    { label: 'Product / Machine Code', fieldName: 'Name', editable: true },
    { label: 'Product Name', fieldName: 'Product_Name__c', editable: true },
    { label: 'Power in HP', fieldName: 'Power_in_HP__c', editable: true },
    { label: 'Unit Price', fieldName: 'Unit_Price__c', editable: true },
    { label: 'Quantity', fieldName: 'Quantity_In_no_s__c', editable: true },
    { label: 'Discount', fieldName: 'Discount__c', editable: true }
];
...
export default class QuotelineItemComponent extends NavigationMixin(LightningElement) {
    ...
    handleSave() {
        const selectedProductIds = this.selectedRows.map(row => row.Id);

        console.log('Selected product IDs:', selectedProductIds);

        // Prepare the records for update
        const productsToUpdate = selectedProductIds.map(productId => {
            return { Id: productId, Orderid: this.recordId };
         });

        console.log('productsall', productsToUpdate);

        updateOrderProducts({ selectedProductIds: selectedProductIds, OrderId: this.recordId })
            .then(() => {
                ...
                title: 'Success',
                message: 'Opportunity related Products Added successfully.',
                variant: 'success',
            });
